...
app = modal.App("viral-post-inference-v5")
...
# Volumes
checkpoint_vol = modal.Volume.from_name("viral-checkpoints-v5", create_if_missing=False)

@app.function(
    image=inference_image,
    gpu="A100",
    volumes={"/checkpoints": checkpoint_vol},
    scaledown_idle_timeout=300,
)
def generate_post(platform: str = "twitter", hook_type: str = "question",
    ...
    """Generate a viral post using the v5 fine-tuned model."""
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="/checkpoints/final_v5",
        max_seq_length=2048,
        dtype=None,
        load_in_4bit=True,
    )
