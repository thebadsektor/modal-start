...
def finetune(config: TrainingConfig):
    ...
    # Load dataset from volume - ensure correct path
    ...
    # Training args - IMPROVED FOR STABILITY
    print("Setting up training arguments...")
    training_args = TrainingArguments(
        output_dir="/checkpoints/final",
        num_train_epochs=config.num_epochs,
        per_device_train_batch_size=config.batch_size,
        gradient_accumulation_steps=8,  # Increased from 4 for stability
        ...
        report_to="none",
        eval_strategy="no",
        max_grad_norm=0.3,  # Stricter clipping from 1.0
    )
